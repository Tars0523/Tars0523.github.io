...
**outlier process란?**  
$\Phi_\rho(w_i)$는 weight $w_i$가 0 또는 1이 아닌 애매한 중간값에 머물 때 패널티를 부과하는 함수다. 직관적으로는 "weight가 애매하게 0.5 같은 데 있으면 벌점을 물린다"는 뜻이다. 이 term 덕분에 최적화 과정에서 $w_i$는 자연스럽게 inlier면 1, outlier면 0으로 수렴하게 된다. 즉 **outlier process = weight의 이진화를 유도하는 regularizer**라고 이해하면 된다.
